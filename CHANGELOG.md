...
- A guided Vote End Time flow for `/edit_vote`: quick options (End Now, In 5 Minutes, In 1 Hour, In 1 Day) plus a Custom Time... option accepting a single pasted Discord timestamp (e.g. `<t:1785639600:F>`, generated by typing `@time` in any normal Discord message box), replacing the old free-text and Date/Time-field designs. Every deadline is still stored and scheduled internally as UTC.
- `/setup` now begins by asking where WASH should create its home channel (a new channel, default name "Watch Party", or an existing one). Every collection's suggestion thread, and by default the watched-movie destination thread, is created as a sibling thread under this one channel -- collections default to threads rather than separate channels.
