...
Run `/database manage` for a single guided entry point instead of remembering which direct subcommand to use: choose a collection from the same picker `/database move`/`backup`/`reset`/`remove` use, then choose an action from a menu -- **Move Collection**, **Edit Collection**, **Backup Collection**, **Restore Collection**, **Reset Collection**, **Remove Collection**, or **Cancel**. Move/Backup/Reset/Remove run the exact same logic as their direct subcommand (nothing about their behavior differs based on which path reached them). Edit Collection opens the same per-collection settings menu `/config`'s Collections section already uses -- Suggestion Destination, Watched Item Archive, Candidate Selection -- with its Back button returning to this menu instead of `/config`'s own collection list. Restore Collection can't be driven from a button click at all (Discord doesn't allow attaching a file upload in response to one), so it points at running `/database restore` directly, the same way the Setup Wizard's "Import Existing Database" option points at `/import`. The direct subcommands (`/database move`, `/database backup`, `/database restore`, `/database reset`, `/database remove`) remain available as shortcuts and are unchanged by `/database manage`'s existence.
...
- **Use Defaults**, which applies the configured candidate count, configured duration, the server's configured default visibility, and the target collection's own configured Candidate Selection Mode, unchanged.
- **Customize This Vote**, which first shows a dropdown to optionally override the target collection's Candidate Selection Mode for this one round only -- the collection's own saved setting is never changed by this, and it defaults to whatever the collection is currently configured with. Pressing **Continue to Vote Settings** then opens a modal accepting a candidate count and blind or visible voting (leaving either blank also uses the server's configured default, not a hardcoded value), plus a duration field using WASH's one shared duration syntax (see "Duration syntax" below) -- anywhere from 1m-30d, with an explicit unit always required (e.g. `10m`, `4h`, `3d` -- a bare `3` is no longer accepted). Practical shortcuts: **10m**, **30m**, **1h**, **12h**, **7d**, or any custom value in that range. Every field's "leave blank to use the configured default" placeholder also names the actual value that will be used (e.g. "Leave blank to use the configured default (Visible)"), so nothing has to be guessed or looked up separately. A reminder-before-close override is also available here, using the same duration syntax with minute precision (e.g. `10m`, `1h`).
...
Each database's `suggestion_rules.candidate_selection` setting chooses how `/vote start` picks nominees from that database's eligible suggestions. It's configured through the Setup Wizard's Voting Defaults step (where, during first-time setup, exactly one collection exists so far) or, afterward, through `/config`'s Collections section -- select the collection, then its Candidate Selection setting; both show and save the exact same value, chosen from a Discord dropdown rather than typed. The Setup Wizard and `/config` present these three modes under friendlier names; the underlying value in parentheses is what's actually persisted:

- **Balanced Random** (`rotation_pool`, the recommended and default choice) -- every eligible suggestion belongs to a rotation. Once presented in a vote, a suggestion is excluded from selection until the rotation is exhausted and a fresh one begins automatically. *Prioritizes suggestions that have appeared in fewer recent votes, giving every one a fair chance.*
...
The italicized sentence after each mode is the exact wording shown as that option's description everywhere it's chosen -- the Setup Wizard, `/config`'s Candidate Selection screen, and `/vote start`'s Customize This Vote -- so an administrator never has to already understand the underlying algorithm to pick one.
...
A rotation also completes early -- before every assigned suggestion has reached one of those states -- whenever a candidate request (from `/vote start` or `/list`) needs more candidates than the current rotation has left to present. A collection with plenty of "Available" suggestions can still have most of them on Rotation Cooldown from a recent round; rather than blocking the next vote until the rotation is fully exhausted, WASH starts a fresh rotation immediately, returning every non-Vote-Winner, non-Retired suggestion (including the ones just on cooldown) to eligibility, and reports the refreshed pool -- never a stale one. This only happens when doing so would actually help -- a rotation is never restarted while it can already supply the requested number of candidates, and a genuinely small collection is reported as having too few eligible suggestions rather than restarting pointlessly. The decision (resolve the current rotation, check whether it already has enough, roll over only if that would help, otherwise report the actual shortage) always runs *before* eligibility is reported back to a command, so a user is never left to separately notice that a rollover was needed -- see "Rotation Refresh Notification" below for how WASH actually tells the user this happened.
...
`CollectionEligibilityService` is the one authoritative implementation of "what suggestions are eligible right now" for a collection -- every command that needs an eligibility answer (`/vote start`, `/list`, `/database health`, and the Rotation Low-Pool Notification below) calls it rather than computing its own. It sits directly on top of the existing candidate-selection strategies (`services/candidate_selection_strategy.py`), so it can never drift from what `/vote start` itself would nominate. It exposes two operations with deliberately different mutation guarantees:

- **Resolve** -- may trigger the automatic rollover described above, because reporting an accurate answer sometimes requires it. Used by `/vote start` and `/list`.
- **Peek** -- strictly read-only; never bootstraps or advances a rotation, even implicitly. Used by `/database health` and the Rotation Low-Pool Notification, both of which must never change what a subsequent `/vote start` or `/list` sees just because someone checked status.

Both operations report the same reconciled buckets for a collection: **Eligible** (selectable right now), **Rotation Cooldown** (presented already this rotation, not yet selectable), **Vote Winners**, and **Retired**. These always reconcile: Active = Eligible + Rotation Cooldown, and Total = Active + Vote Winners + Retired.

`/database health` (WASH Crew only) reports this breakdown for one collection: Collection Name, Total Watch Items, Active Watch Items, Eligible for Voting, Rotation Cooldown, Vote Winners, Retired, the collection's own current Rotation number and progress (e.g. "Rotation 4 Progress: 18 of 27 active items have been presented"), the guild's Configured Candidate Count, a **Next Vote** status, and a **Low Pool Status**. Eligible for Voting and Rotation Cooldown are shown indented beneath Active Watch Items, so the reconciliation identity (Active Watch Items = Eligible for Voting + Rotation Cooldown) is visible directly in the layout, not just stated in a parenthetical. Collection selection works exactly like `/list`'s -- automatic from thread context, with a **Switch Collection** button when more than one collection exists. Because it uses Peek, running `/database health` never changes what `/vote start` or `/list` will subsequently see -- checking health is always side-effect-free.

**Rotation numbering.** The rotation number shown (e.g. "Rotation 4") is this specific collection's own 1st/2nd/3rd/... rotation, not the raw internal rotation record ID -- that ID is a single counter shared across every collection on the server, so showing it directly would read as an arbitrarily large, confusing number for a collection's very first rotation. `/database health` is the only place a rotation number is shown; it's deliberately not added to every voting-related embed, to avoid clutter members don't need.

**Next Vote** is one of:

- **Ready** -- the current rotation already has enough eligible suggestions to satisfy the configured candidate count.
- **Needs Rollover** -- not enough are eligible right now, but the collection has enough active (eligible + cooldown) suggestions overall that starting a vote would trigger the automatic rollover above and succeed.
- **Insufficient Suggestions** -- the collection doesn't have enough active suggestions to satisfy the candidate count even after a rollover; more suggestions need to be added.

**Low Pool Status** is one of **Healthy**, **Almost Complete**, or **Insufficient**, using the same threshold the Rotation Low-Pool Notification below evaluates against -- Collection Health and the notification can never disagree about what "low" means.

### Rotation Refresh Notification

Whenever `/vote start` or `/list` actually rolls a completed rotation over (see "Rotation lifecycle" above), WASH says so plainly, rather than leaving a WASH Crew member to notice the pool composition changed on its own:

> All eligible watch items have now been presented.
>
> Starting Rotation 4.

This is purely informational, never warning-toned -- rolling a rotation over is normal, expected behavior, not a problem. For `/vote start`, it's sent as a follow-up message visible only to the WASH Crew member who ran the command, after the public voting post itself -- the public post never mentions it, so every other voter sees a clean voting post with no rotation bookkeeping in it. For `/list`, it's shown as a leading line in the same response, since `/list`'s own output is already private to the viewer by default. It never fires for `/database health` (which never rolls a rotation over at all) or for a `/list` filter that only reads terminal statuses (Vote Winners, Retired), since neither ever bootstraps or advances a rotation.

### Rotation Low-Pool Notification

WASH can proactively notify WASH Crew when a collection's eligible pool is running low, rather than requiring someone to check `/database health` or wait for `/vote start` to come up short. Configured per-server through `/config`'s Rotation Low-Pool Notification section:

- **Enabled/Disabled** -- on by default.
- **Threshold** -- the number of eligible suggestions below which the notification fires. Defaults to the larger of **10% of the collection's Active Watch Items** or **two configured voting rounds** (2 × the guild's configured candidate count), computed dynamically against the collection's own current size so it scales sensibly for both small and large collections; a server may instead set a fixed custom threshold, or restore the automatic default.
- **Destination** -- the Admin Channel (default) or the Watch Party Home Channel.

The percentage term only ever counts **Active Watch Items** (Eligible for Voting + Rotation Cooldown) -- Vote Winners and Retired are always excluded, since they're no longer part of the pool that could ever become eligible again. For a small collection, the two-voting-rounds floor almost always governs; for a large collection, the 10% term scales the threshold up so a large collection isn't warned about prematurely just because its rounds-based floor is a small, fixed number.

**Suppression for small collections.** If a collection's entire Active pool is already at or below the computed threshold, the notification never fires for it at all -- not because the pool isn't low, but because a collection that small would trigger the notification on every single rotation forever, with nothing new to report each time. `/database health`'s own Low Pool Status still reports accurately for these collections (typically **Insufficient**); only the *proactive* notification is suppressed.

The notification is evaluated using Peek, so simply checking the pool size is never itself enough to trigger a rollover -- it fires only when the eligible count is genuinely below the threshold right now. It's currently evaluated after a successful `/vote start` or `/add`, the moments a database's pool most naturally changes. It fires **once per rotation**: WASH remembers which rotation it already notified for, and naturally stops repeating until a fresh rotation begins (at which point the new rotation's different identifier means it's due again) -- there's no separate "reset" step. Collections using Pure Random (`infinite_pool`) never have rotation state, so this notification never fires for them. The message names the collection, the remaining eligible count, the configured candidate count, and a nudge to use `/add`.

**Migrating from the old Low Pool Reminder.** This replaces the previous interval-based reminder (fixed default threshold of 10, minimum 24-hour resend interval, evaluated after `/add`). A server with a previously saved custom threshold keeps that exact number as its new Rotation Low-Pool Notification threshold; a server that never explicitly saved one now gets the new dynamic, size-aware default described above instead of the old flat 10. The old reminder's enabled/disabled setting is preserved as-is. The old interval-based resend timer is gone entirely, replaced by the once-per-rotation behavior above.

### Known limitations: candidate selection

- **Pure Random collections never receive a Rotation Low-Pool Notification.** `infinite_pool` collections have no rotation to key a once-per-rotation notification on; `/database health`'s Low Pool Status still reports accurately for them, but no proactive notification is sent. Checking `/database health` periodically is the only option for these collections today.
