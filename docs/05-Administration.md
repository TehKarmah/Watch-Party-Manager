...
`/database health` (WASH Crew only) reports this breakdown for one collection: Collection Name, Total Watch Items, Active, Eligible Next Round, Rotation Cooldown, Vote Winners, Retired, Current Rotation Progress (e.g. "18 of 27 active items have been presented"), the guild's Configured Candidate Count, a **Next Vote** status, and a **Low Pool Status**. Collection selection works exactly like `/list`'s -- automatic from thread context, with a **Switch Collection** button when more than one collection exists. Because it uses Peek, running `/database health` never changes what `/vote start` or `/list` will subsequently see -- checking health is always side-effect-free.
