...
- 🗳️ **In an Active Vote** (Rotation-removal Phase 1) -- currently a candidate in an open voting round for its collection. Computed at display time from `VoteService`, the same way Rotation Cooldown is computed -- there is no separately stored value. This is the preferred status for a suggestion currently up for a vote; it takes precedence over Rotation Cooldown whenever both would otherwise apply.
- 🟡 **Rotation Cooldown** -- already presented in the database's current rotation; automatically returns to Available the moment a fresh rotation begins. This is computed at display time, not a separately stored value, so there is no manual "clear cooldown" step. Only relevant for a database still using the Balanced Random or Soft Rotation nominee-selection modes (see "New suggestion admission" above).
...
`CollectionEligibilityService` is the one authoritative implementation of "what suggestions are eligible right now" for a collection -- every command that needs an eligibility answer (`/vote start`, `/list`, `/database health`, and the Rotation Low-Pool Notification below) calls it rather than computing its own. It sits directly on top of the existing nominee-selection strategies (`services/candidate_selection_strategy.py`), so it can never drift from what `/vote start` itself would nominate. It exposes two operations with deliberately different mutation guarantees:
...
- **Pure Random, Favor New Additions, and Favor Older Additions collections never receive a Rotation Low-Pool Notification.** None of these three modes create rotation state to key a once-per-rotation notification on; `/database health`'s Low Pool Status still reports accurately for them, but no proactive notification is sent. Checking `/database health` periodically is the only option for these collections today.
