...
Core functionality is implemented and tested; WASH is in release-candidate hardening. Recent work: **Rotation-removal Phase 2** -- the live user experience finished transitioning to the new eligibility model while `RotationService`/`RotationRepository`/rotation persistence all remain in place internally (not yet deleted). Rotation Cooldown is gone from every user-facing surface; the five authoritative states are now Available, In an Active Vote, Vote Winner, Retired, and Watched. `CollectionEligibilityService` is now computed entirely from suggestion status and `VoteService` -- it no longer touches `RotationService` at all, so `/list` and `/database health` never trigger, check, or report a rotation rollover any more (a legacy Balanced Random/Soft Rotation collection's own internal rollover still happens, silently, only at actual vote-creation time, and is announced only via `/vote start`'s Rotation Refresh Notification). The old Rotation Low-Pool Notification was replaced by the **Eligible Pool Warning**: threshold is now a flat `candidate_count × 5` (was the larger of 10% of Active Watch Items or two configured voting rounds), and its once-per-rotation dedup was replaced with threshold-crossing dedup (fires once, suppresses while still at/below threshold, re-arms once back above it, fires again on the next drop) tracked in its own rotation-independent state file, so it now fires equally for every nominee-selection mode. `/database health`'s Next Vote status lost its "Needs Rollover" state (folded into Ready, since a legacy rollover is now always silent); its Rotation Progress line is omitted entirely for the three rotation-free modes instead of showing "N/A". Earlier work, Rotation-removal Phase 1: two new, rotation-free nominee-selection modes (Favor New Additions, Favor Older Additions) were added alongside the existing three, using each suggestion's permanent suggestion date rather than rotation state, and are now the only modes offered when configuring a collection for the first time (Favor New Additions is the new default); new suggestions became eligible immediately regardless of mode; and "Candidate Selection" was renamed to "Nominee Selection" throughout the UI and documentation (internal class/file names are unchanged). Earlier still: a four-status suggestion model (Available/Rotation Cooldown/Vote Winner/Retired) with synchronized public-post status fields, a redesigned resumable `/setup` wizard (Home Channel step, collections defaulting to sibling threads under it), a standardized duration syntax (`10m`/`1h`/`7d`, an explicit unit always required; `w`/week(s) is also accepted but never advertised) shared by vote duration, reminder-before-close (now minute-precise), and `/voting edit`'s Shorten/Extend Vote, a full terminology/navigation/consistency audit pass, and (pre-release, breaking) a command structure cleanup: `/start_vote`/`/vote_status`/`/edit_vote` became `/voting start`/`/voting status`/`/voting edit`; `/database_add`/`/database_list`/`/database_backup`/`/database_restore`/`/database_reset`/`/database_remove` became `/database add`/`/database list`/`/database backup`/`/database restore`/`/database reset`/`/database remove`, alongside a new `/database move` and a modernized `/database add` (type choice, then a destination choice: Create New Thread, Use Current Thread/Channel, Use Existing Thread, or Use Existing Channel -- the same shared destination-selection flow `/database move` and the new guided `/database manage` command both reuse); and `/schedule_watch_party`/`/reschedule_watch_party`/`/cancel_watch_party`/`/watch_party_status` became `/watch-party schedule`/`/watch-party reschedule`/`/watch-party cancel`/`/watch-party status` (hyphenated, distinct from the pre-existing `/watch_party` membership-administration group). No compatibility aliases were kept.
...
| Nominee selection & rotation | Complete; Rotation-removal Phase 1 underway | Favor New Additions (default) and Favor Older Additions are the only modes offered to new collections; Rotation Pool, Soft Rotation, and Infinite Pool remain fully functional for any collection already configured with one of them. New suggestions are eligible immediately regardless of mode. Persistent rotation lifecycle tracking (for the two legacy modes), Low Pool Reminder. |
...
- `RotationService`, nominee selection strategies (Favor New Additions / Favor Older Additions / Infinite Pool / Rotation Pool / Soft Rotation)
...
- A suggestion's five display statuses are Available, In an Active Vote, Vote Winner, Retired, and Watched (Rotation-removal Phase 2 removed the sixth, Rotation Cooldown, from the user-facing model entirely) -- `WatchItemStatus.VOTE_WINNER` is set automatically when a voting round completes; a group's actual viewing is separately confirmed through the watch-history workflow.
