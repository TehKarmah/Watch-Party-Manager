...
When starting a vote, an administrator may optionally narrow the Eligible Pool further for that one round only, before the collection's configured selection mode runs, using any combination of five filters: Genre (one genre already present in the pool's stored metadata), IMDb Rating (an optional minimum and/or maximum), MPAA Rating (one content rating already present in the pool), Actor (one cast member, found via a case-insensitive search of already-stored cast metadata), and Suggestion Source (suggestions originally submitted by one chosen member, e.g. for a birthday vote, with no separate profile or birthday record required). Every filter defaults to no narrowing and is never saved to the collection's own configuration -- the collection's selection mode remains one of the three standard modes above regardless of any per-vote narrowing applied. A suggestion missing the metadata a given filter checks is treated as not matching that filter while it is active, but remains fully eligible whenever that filter is not in use. The same five filters, with identical behavior, are also available to `/random watch`'s discovery pick, which otherwise bypasses the collection's configured selection mode entirely in favor of a uniform random choice.
