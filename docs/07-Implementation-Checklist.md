...
| FR-002 | Implement the Watch Item Journey domain model | Complete foundation | Rejection/retirement and vote-win recording are wired into voting completion. Automatic "watched" marking is not yet produced by any code path. |
...
| FR-005 | Implement nominee selection | Complete | Guild-scoped selection with configurable Pure Random / Favor New Additions / Favor Older Additions strategies and candidate-count validation. |
| FR-006 | Implement voting rounds and ballots | Complete foundation | Blind/visible modes, duration, vote changes, persistence, and ballot validation are implemented. Visible is the default (Blind remains fully selectable); a vote started without an explicit override uses the guild's configured default. Duration is minute-based (1 minute - 30 days, default 1440 minutes/1 day); older hour- or day-based guild configurations load as the equivalent minute count. |
...
| FR-010 | Implement watch history | Partial | Winner recording into Watch Item Journey (times won) is implemented. Marking an item "watched" and retroactive correction are not yet implemented. |
...
| FR-013 | Expand statistics and reporting | Complete foundation | Server, member, suggestion, and database statistics are implemented via `/stats`, derived from historical data with no running counters. |
...
FR-017 and later (membership workflows, IMDb link normalization/duplicate detection, nominee selection, statistics, and this documentation pass) are implemented but not yet individually broken out into this table -- see `project_state.md`'s Functional Requirement Status for the current, area-based summary.
