...
`/database health` (Rotation & Collection Health) reports one collection's eligibility breakdown -- Total Watch Items, Active (Eligible Next Round + Rotation Cooldown), Vote Winners, Retired, the current rotation's progress, the guild's configured candidate count, a **Next Vote** status (Ready / Needs Rollover / Insufficient Suggestions), and a **Low Pool Status** (Healthy / Almost Complete / Insufficient). Collection selection works exactly like `/list`'s (automatic from thread context, with a **Switch Collection** button). Unlike `/list`/`/vote start`, `/database health` never triggers a rotation rollover -- it only ever reports the current state, so simply checking health can never change what a subsequent `/vote start` sees.
