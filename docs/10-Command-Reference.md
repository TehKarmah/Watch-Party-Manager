...
| `/database manage` | WASH Crew | Guided workflow: pick a collection, then choose what to do with it (move, edit, back up, refresh IMDb metadata, recover missing IMDb links, reset, or remove). |
...
`/database manage` is the single guided entry point for every other collection-management action: pick a collection from a picker (name, Active/Inactive status, and watch-item count -- never a typed ID), then choose an action from a menu ordered administrative actions first, destructive actions last: **Edit Collection**, **Backup Collection**, **Move Collection**, **Restore Collection**, **Refresh IMDb Metadata**, **Recover Missing IMDb Links**, **Reset Collection**, **Remove Collection**, or **Cancel**. Edit Collection opens the same per-collection settings menu `/config`'s Collections section already uses (Suggestion Destination, Watched Item Archive, Nominee Selection -- "I Won't Watch" is shown there for reference but edited from its own dedicated `/config` section). Move Collection changes only the chosen collection's suggestion destination -- its database ID, suggestions, statuses, vote history, statistics, and every other setting are untouched, and its existing Discord suggestion posts stay exactly where they are; only suggestions added after the move post to the new destination. Refresh IMDb Metadata re-fetches already-linked suggestions' IMDb-derived fields (title, plot, poster, runtime, genres, IMDb rating, MPAA/content rating, director, cast); choose Refresh This Collection or Refresh All Collections (every active collection in the current server only -- never another server's, even on a WASH process hosting more than one), confirm (shown: scope, collection count, eligible-suggestion count, and that external IMDb/OMDb requests will be made), then Start Refresh. A suggestion missing metadata for a field keeps its existing value rather than having it blanked out; every non-IMDb field (status, history, votes, Discord references) is untouched; a suggestion with no usable IMDb link is skipped, not guessed at. See [Administration: IMDb Metadata Refresh](05-Administration.md#imdb-metadata-refresh) for full details. Recover Missing IMDb Links is the counterpart for suggestions with *no* IMDb link at all: it searches OMDb by each suggestion's stored title/year and, one suggestion at a time, requires an explicit Accept/Skip/Search Again/Cancel Recovery decision before anything is saved -- accepting a match saves the identifier and immediately reuses Refresh IMDb Metadata's own logic to populate the rest. A suggestion that already has a link is never offered here, and an existing link is never overwritten. See [Administration: IMDb Metadata Recovery](05-Administration.md#imdb-metadata-recovery) for full details. Reset Collection and Remove Collection are destructive and require typing a confirmation phrase, kept visually distinct (red/danger-styled buttons) from the administrative actions above them.
