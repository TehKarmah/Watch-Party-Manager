...
`/random_watch` resolves a collection the same way `/list` does (automatic from thread context when unambiguous; a picker when it isn't, offered again from the result screen via **Change Collection**), then shows **Pick Random Item** and **Add Filters**. Selection is always uniformly random over the collection's eligible pool (the same Available items `/list`'s Eligible for Voting and voting itself use) -- it never applies Favor New/Older Additions or any other nominee-selection weighting, and **Pick Again** performs a fresh independent draw with immediate repeats allowed (no hidden no-repeat state). Add Filters optionally narrows the random pool to one member's eligible suggestions (a Discord User Select, matched by stable Discord user ID -- never a display name, so legacy suggestions with no recorded submitter never match) and/or one genre already present in the eligible pool's stored IMDb metadata (case-insensitive, each option showing its eligible count); both may be combined, and apply only to the current session -- they never change the collection's or guild's own configuration. The result shows the chosen item with WASH's usual suggestion details (title, year, poster, genre, IMDb link, original suggester, and a link back to its original suggestion post when available) and always makes clear it was chosen randomly, with **Pick Again**, **Change Filters**, **Change Collection**, and (when available) **View Original Suggestion** buttons. `/random_watch` is a discovery tool only: it never starts a voting round, changes a suggestion's status, marks anything watched, or changes any collection setting.
