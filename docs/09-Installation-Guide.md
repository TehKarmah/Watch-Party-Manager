...
- Send Messages
- Embed Links
- Attach Files (used by `/backup`, Backup Collection (`/database manage`), and `/import` to send/receive `.zip` files)
...
10. **Backup defaults** -- **Enable Automatic Backups** (recommended, offered first) then sets the interval and how many backups to retain, or **Disable Automatic Backups** to skip scheduling entirely for now (manual `/backup` always remains available either way, and existing backups are never deleted by disabling this).
