...
6. **Watched Item Archive** -- where WASH should archive completed watch items: **Create New Thread** (recommended, a brand-new thread created as a sibling under the home channel -- default name "Watched Item Archive", editable, never nested under a suggestion thread), an existing text channel or thread instead, or skip for now. This archive stores Vote Winners and Retired items together with links back to their suggestion and voting history -- it is not a discussion channel.
...
11. **Summary** -- review every section (including the chosen candidate-selection mode and "I Won't Watch" Enable/Disable and threshold), then Save, jump back to edit any section, or cancel without saving.
