...
7. **Voting defaults** -- candidate count and duration are entered in a modal (default 3 candidates; duration 1m-30d, default **1 day**, entered as a whole number immediately followed by a unit -- minutes, hours, or days, e.g. `10m`, `1h`, or `7d` -- and shown in natural language afterward like "10 minutes", "4 hours", or "3 days"); visibility **Visible** by default, **Blind** remains fully selectable -- explained right on this step: Visible means everyone can see vote totals while voting is active, Blind means results stay hidden until voting closes. Candidate-selection mode is chosen from a dropdown, not typed, each option showing a short explanation of what it does: **Favor New Additions** (recommended and the default -- leans toward suggestions added recently, older ones stay eligible at a lower chance), **Favor Older Additions** (leans toward suggestions that have waited the longest, newer ones stay eligible at a lower chance), or **Pure Random** (chooses completely at random, with no preference or exclusion).
