...
7. **Voting defaults** -- candidate count, duration/visibility, and the collection's "I Won't Watch" threshold are entered in a modal (default 3 candidates; duration 1m-30d, default **1 day**, entered as `10m`, `1h`, or `7d` and shown in natural language like "10 minutes", "4 hours", or "3 days"; visibility **Visible** by default, **Blind** remains fully selectable -- explained right on this step: Visible means everyone can see vote totals while voting is active, Blind means results stay hidden until voting closes; "I Won't Watch" threshold 1-10, default **2** -- the number of distinct members who must press "I Won't Watch" before a suggestion needs WASH Crew review). Candidate-selection mode is chosen from a dropdown, not typed, each option showing a short explanation of what it does: **Favor New Additions** (recommended and the default -- leans toward suggestions added recently, older ones stay eligible at a lower chance), **Favor Older Additions** (leans toward suggestions that have waited the longest, newer ones stay eligible at a lower chance), or **Pure Random** (chooses completely at random, with no preference or exclusion).
8. **Reminder defaults** -- **Enable Vote-Ending Reminder** (recommended, offered first) then sets how long before close it's sent (default **1 day**, entered as `10m`, `1h`, or `7d`), or **Disable Vote-Ending Reminder** to skip that step entirely for this pass (a previously configured lead time, if any, is left untouched and simply becomes inactive).
...
10. **Summary** -- review every section (including the chosen candidate-selection mode and "I Won't Watch" threshold), then Save, jump back to edit any section, or cancel without saving.
...
Every setting the wizard can set (including candidate-selection mode and the "I Won't Watch" threshold) can also be changed later through `/config`, one section at a time, without re-running the whole wizard -- see Section 12.
