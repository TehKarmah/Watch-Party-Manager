...
4. **Home channel** -- where WASH creates its home. Choose **Create New Channel** (recommended; defaults to "Watch Party", editable) or **Use Existing Channel**. Every collection's suggestion thread, and by default the watched-movie destination thread, is created as a sibling thread under this one channel -- for example:
