...
- 🍿 **Watch Party** -- members who submit suggestions, browse collections, vote, and participate in movie nights.

Create both roles in your server's **Server Settings → Roles** before starting `/setup`. If you've never created a Discord role before, see Discord's own [Roles and Permissions guide](https://support.discord.com/hc/articles/214836687-Discord-Roles-and-Permissions). The server owner never needs either role themselves -- WASH always grants the owner every member-level ability (adding suggestions, browsing, voting, Random Watch) automatically.
...
- Send Messages
- Embed Links
- Attach Files (used by `/maintenance backup`, Backup Collection (`/database manage`), and `/maintenance import` to send/receive `.zip` files)
- Read Message History (used to edit WASH's own suggestion, vote, and confirmation posts)
- Manage Channels (the Setup Wizard's Home Channel and Admin Channel steps can create a new channel for you; without this permission, select an existing channel from the picker in those steps instead)
...
**Preparation screen.** The first screen you see (only on a brand-new run, not when resuming a saved draft) is a preparation screen, not one of the numbered steps below. It reminds you that both Discord roles (🛠️ WASH Crew and 🍿 Watch Party -- see Before You Begin, above) should already exist before you continue, and that setup can be paused with Save & Finish Later and that existing channels or threads can be reused wherever the wizard asks for a destination. Press **Begin Setup** to continue, or **Cancel** to stop without starting a draft.

The wizard then walks through, in order:

1. **🛠️ WASH Crew role** -- select the role, review your selection, then press **Save & Continue** (selecting a role no longer advances automatically). **Save & Finish Later** is also available here. A role must actually be selected before Save & Continue proceeds -- WASH shows a clear message if you try to continue with nothing chosen.
2. **🍿 Watch Party role and join mode** -- until it's configured, only WASH Crew (and the server owner, who always has member-level access regardless of role) can use WASH's member commands. Also choose how members get it: Self-Service (anyone can join with `/join watch party`), Manual (WASH Crew adds members), Approval-Required (requests go to WASH Crew for approval), or Discord-Managed (an existing role you manage outside WASH).
3. **Admin channel** -- where Approval-Required membership requests are posted for WASH Crew. Select an existing channel and press **Save & Continue** (selecting alone doesn't advance -- you can freely change your mind before confirming), create a new private channel for it, or skip for now. A private channel will appear in the picker once WASH has been granted **View Channel** and **Send Messages** on it -- grant those, then reopen this step to see it listed. If WASH also assigns server roles, its own role must sit above those roles in the server's role hierarchy.
4. **Home channel** -- where WASH creates its home. Select an existing text channel and press **Save & Continue**, or choose **Create New Channel (Recommended)** (defaults to "Watch Party", editable) -- both offered on the same screen. Every collection's suggestion thread, and by default the Watched Item Archive thread, is created as a sibling thread under this one channel -- for example:
...
5. **Collection** -- **Create New** (recommended, offered first) walks you through a guided prompt: **Movies** or **TV Shows** (creates a collection with a descriptive default thread name -- "Movie Suggestions" or "TV Suggestions" -- with no further naming needed), **Special Collection** or **Custom** (asks for a name first). WASH creates that collection's suggestion thread directly as a sibling under the home channel and saves it immediately as the collection's Suggestion Destination -- `/add` works in it right away, no further configuration needed. **Select Existing** picks one already configured in this server instead. **Import Existing Database** points you at running `/maintenance import` separately, since Discord doesn't allow attaching a file from inside this wizard. A collection's suggestion destination can be changed later via `/config`, but never cleared -- every collection always has exactly one.
6. **Watched Item Archive** -- where WASH should archive completed watch items: select an existing text channel or thread and press **Save & Continue**, choose **Create New Thread (Recommended)** (a brand-new thread created as a sibling under the home channel -- default name "Watched Item Archive", editable, never nested under a suggestion thread), or skip for now. This archive stores Vote Winners and Retired items together with links back to their suggestion and voting history -- it is not a discussion channel.
7. **Voting defaults** -- candidate count and vote duration (how long voting stays open -- not the movie's runtime) are entered in a modal (default 3 candidates; duration 1m-30d, default **1 day**, entered as a whole number immediately followed by a unit -- minutes, hours, or days, e.g. `10m`, `1h`, or `7d` -- and shown in natural language afterward like "10 minutes", "4 hours", or "3 days"); visibility **Visible** by default, **Blind** remains fully selectable -- explained right on this step: Visible means everyone can see vote totals while voting is active, Blind means results stay hidden until voting closes. Candidate-selection mode is chosen from a dropdown, not typed, each option showing a short explanation of what it does: **Favor Older Additions** (recommended and the default -- leans toward suggestions that have waited the longest, newer ones stay eligible at a lower chance), **Favor New Additions** (leans toward suggestions added recently, older ones stay eligible at a lower chance), or **Pure Random** (chooses completely at random, with no preference or exclusion).
8. **"I Won't Watch" Settings** -- its own dedicated step, immediately after Voting Defaults: **Enable "I Won't Watch" (Recommended)** then sets the threshold, 1-10, default **2** -- the number of distinct members who must press "I Won't Watch" before a suggestion needs WASH Crew review -- or **Disable "I Won't Watch"** to turn the whole system off for this collection (a previously configured threshold, if any, is left untouched and simply becomes inactive; the suggestion post's "I Won't Watch" button disappears entirely while disabled). The step explains this with a concrete example (e.g. "threshold 2 means the suggestion is flagged for review once 2 different members choose 'I Won't Watch'") and makes clear that reaching the threshold never removes the suggestion from the collection -- only from eligibility for future votes, until WASH Crew reviews it. `/config`'s own "I Won't Watch" Settings section explains it identically.
9. **Reminder defaults** -- **Enable Vote-Ending Reminder** (recommended, offered first) then sets how long before close it's sent (default **1 day**, same minutes/hours/days syntax as Voting Defaults, e.g. `10m`, `1h`, `7d`), or **Disable Vote-Ending Reminder** to skip that step entirely for this pass (a previously configured lead time, if any, is left untouched and simply becomes inactive).
10. **Backup defaults** -- **Enable Automatic Backups** (recommended, offered first) then sets the interval and how many backups to retain, or **Disable Automatic Backups** to skip scheduling entirely for now (manual `/maintenance backup` always remains available either way, and existing backups are never deleted by disabling this). This backs up WASH's own data only -- suggestions, votes, collections, and server configuration -- never Discord itself (messages, channels, roles, and permissions are Discord's own responsibility and are never included).
11. **Summary** -- review every section (including the chosen candidate-selection mode and "I Won't Watch" Enable/Disable and threshold), then Save, jump back to edit any section, or cancel without saving. Save & Finish Later is not offered here -- every section has already been answered by this point, so there's nothing left to save and come back to.

Nothing is applied until you reach the summary and choose to save.

**Back, and Save & Finish Later.** Every step after the first shows a **Back** button that returns to the immediately previous step without losing anything you've already entered -- the first step has no Back button, since there's nothing earlier to return to. Every step except the final Summary also shows a **Save & Finish Later** button: it saves your progress so far, exits the wizard cleanly, and does *not* mark setup as complete. That confirmation screen offers its own **Continue Setup** button to resume immediately, and also reminds you that running `/setup` again later resumes exactly where you left off. **Cancel Setup**, by contrast, discards everything entered so far -- use Save & Finish Later if you just want a break.

**Resuming.** If you run `/setup` again while a draft is already saved, WASH shows how many of the 11 steps are done and where you left off, then offers **Continue Setup** (pick up where you stopped), **Review Progress** (jump straight to the summary screen), or **Restart Setup** (discard the saved draft and start over -- never the default choice). Once setup has actually been completed and saved, `/setup` no longer offers to resume or restart at all; it redirects you to `/config` instead.

Every setting the wizard can set (including candidate-selection mode and "I Won't Watch" Enable/Disable and threshold) can also be changed later through `/config`, one section at a time, without re-running the whole wizard -- see Section 12.
...
- `/config` opens the same settings in a menu, section by section, for quick individual changes -- including an **Eligible Pool Warning** section not covered by the setup wizard: enable/disable, a custom eligible-suggestion threshold (or restore the automatic default of the configured candidate count times 5), and a destination (Admin Channel, the default, or the Watch Party Home Channel). "I Won't Watch" Settings (Enable/Disable and threshold) has its own dedicated section too, immediately after Voting Defaults, mirroring the wizard's own step. Every change saves immediately, so the menu's own closing button is labeled **Done**, not a save-implying label. Any section left unconfigured or pointing at a role/channel that no longer exists is clearly flagged with a ⚠️ warning marker (further marked (Required) or (Optional) where relevant), with a summary banner at the top of the menu whenever anything needs attention.
- `/database add` and `/database list` create and list additional suggestion databases beyond the one created during setup. `/database manage` is the single guided menu for every other collection action (move, edit, back up, restore, reset, or remove a chosen collection) -- those actions have no separate top-level command; `/database restore` is the one exception, since restoring needs a file upload. `/database health` reports a collection's eligibility and pool health without changing anything.
- `/membership` (WASH Crew only) manages membership directly: list, approve/deny pending requests, manually add or remove members, and search a member's history.
...
| The Setup Wizard's "Create New Channel" option fails with a permission message | WASH wasn't invited with Manage Channels (see Section 6). | Grant WASH the Manage Channels permission and try again, or select an existing channel from that step's picker instead. |
| Changes made via `/maintenance restore`, `/database restore`, Reset Collection (`/database manage`), `/maintenance reset`, or `/maintenance import` don't seem to take effect | Several repositories cache their data in memory at startup. | Restart WASH after any of these commands -- see [Administration](05-Administration.md)'s "Backup & Recovery" section for the full explanation. |
