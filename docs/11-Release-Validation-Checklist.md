...
- **Steps:**
  1. Type `/database` in Discord's command box; confirm autocomplete offers exactly `add`, `list`, `health`, `manage`, `restore` as subcommands -- `move`, `backup`, `remove`, and `reset` are reached only through `manage`'s own action menu, not as top-level subcommands -- and that typing `/database_add`, `/database_backup`, `/database_restore`, `/database_reset`, `/database_list`, or `/database_remove` (the old top-level names) finds nothing.
  ...
  3. Type `/watch-party`; confirm it does **not** appear in autocomplete at all -- `WatchPartyEventGroup` (`schedule`/`status`/`reschedule`/`cancel`) is deliberately not registered for v1 (see Section 6's deferral note). Do not treat its absence as a failure.
  4. Type `/membership`; confirm it still exists, separately from the (unregistered) `/watch-party` name, and still offers `members`, `pending`, `approved`, `denied`, `add`, `remove`, `search` -- membership administration is unaffected by this cleanup.
...
- **Expected Result:** The live Discord command tree matches [Command Reference](10-Command-Reference.md) exactly -- no obsolete command remains reachable under its old name, no command was accidentally duplicated under both an old and new name, `/membership` remains its own distinct, valid command name, and `/watch-party` does not appear at all (deferred from v1).
- **Result:** [ ] Pass [ ] Fail
- **Notes:** ___________________________
