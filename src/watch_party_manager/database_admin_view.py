...
Use Existing Thread, or Use Existing Channel -- so it's built once here
and reused by both, rather than duplicated. Like setup_wizard_view.py,
...
class UseExistingChannelButton(discord.ui.Button):
    def __init__(self, on_click: OnDestinationChoice) -> None:
        super().__init__(
            ...
            style=discord.ButtonStyle.secondary,
            custom_id="wpm_database_admin_destination_existing_channel",
        )
        self._on_click = on_click
...
class UseCurrentLocationButton(discord.ui.Button):
    """Use whichever thread or channel the command was actually run in.

    disabled (not omitted) when the invoking location isn't a usable
    text channel or thread (e.g. run somewhere Discord doesn't support
    suggestion destinations) -- the option stays visible for consistency
    across every invocation, just unavailable, rather than shifting the
    other buttons around.
    """

    def __init__(self, on_click: OnDestinationChoice, *, disabled: bool = False) -> None:
        super().__init__(
            label="Use Current Thread/Channel",
            style=discord.ButtonStyle.secondary,
...
class DestinationChoiceView(discord.ui.View):
    ...
    Thread/Channel, Use Existing Thread, Use Existing Channel, or Cancel.
    """
    ...
    def __init__(
        ...
        on_use_existing_channel: OnDestinationChoice,
        on_cancel: OnCancel,
        *,
...
# ExistingChannelSelectView (Use Existing Channel: text channels only) is
# not redefined here -- the Setup Wizard's own view already does exactly
# this (see setup_wizard_view.py), so it's imported and re-exported
