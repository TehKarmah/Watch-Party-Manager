...
Rotation-removal Phase 2: this file used to also persist a small
per-database "which rotation was the Rotation Low-Pool notification last
sent for" map. That map is gone -- the Eligible Pool Warning that
replaced the Rotation Low-Pool notification dedups on a simple armed/
disarmed flag with no rotation dependency at all (see
persistence/eligible_pool_warning_state_repository.py). A rotations.json
file saved before this change simply has its old
`low_pool_notified_rotation_ids` key ignored on load rather than
migrated -- harmless, since nothing reads it anymore.
"""
...
class JsonRotationRepository:
    ...
    def load(self) -> RotationLoadResult:
        ...
        try:
            ...
            return RotationLoadResult(rotations=rotations, next_rotation_id=next_rotation_id)
        except (json.JSONDecodeError, KeyError, TypeError, ValueError) as exc:
            logger.error(f"Could not load rotation data from {self._file_path}: {exc}")
            return RotationLoadResult(rotations=[], next_rotation_id=FIRST_ROTATION_ID)

    def save(self, rotations: Iterable[Rotation], next_rotation_id: int) -> None:
