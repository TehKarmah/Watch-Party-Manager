...
class VoteCommandTests(unittest.TestCase):
    ...
    # --- /start_vote: nominee count ---------------------------------------

    def test_default_nominee_count_is_used_when_override_is_omitted(self) -> None:
        self.suggestion_service.suggest("Blade Runner")

        message, ephemeral = self._start_vote(default_nominee_count=2)

        self.assertFalse(ephemeral)
        self.assertIn("Candidates: 2", message)
        self.assertEqual(len(self.vote_service.get_open_round().candidate_suggestion_ids), 2)

    def test_custom_nominee_count_overrides_the_default_for_one_round(self) -> None:
        self.suggestion_service.suggest("Blade Runner")

        message, ephemeral = self._start_vote(
            nominee_count=4, default_nominee_count=2
        )

        self.assertFalse(ephemeral)
        self.assertIn("Candidates: 4", message)
        self.assertEqual(len(self.vote_service.get_open_round().candidate_suggestion_ids), 4)

    def test_minimum_nominee_count_is_accepted(self) -> None:
        message, ephemeral = self._start_vote(nominee_count=2)

        self.assertFalse(ephemeral)
        self.assertIn("Candidates: 2", message)

    def test_nominee_count_below_minimum_is_rejected(self) -> None:
        message, ephemeral = self._start_vote(nominee_count=1)

        self.assertTrue(ephemeral)
        self.assertIn("between 2 and 10", message)
        self.assertIsNone(self.vote_service.get_open_round())

    def test_nominee_count_above_maximum_is_rejected(self) -> None:
        message, ephemeral = self._start_vote(nominee_count=11)

        self.assertTrue(ephemeral)
        self.assertIn("between 2 and 10", message)
        self.assertIsNone(self.vote_service.get_open_round())

    def test_invalid_custom_nominee_count_does_not_change_the_default(self) -> None:
        message, ephemeral = self._start_vote(
            nominee_count=11, default_nominee_count=2
        )
