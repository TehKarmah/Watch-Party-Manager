...
class VoteServiceCandidateTests(unittest.TestCase):
    ...
    def test_create_round_rejects_zero_candidate_id_cleanly(self) -> None:
        result = self.service.create_round(candidate_suggestion_ids=[0, 1])

        self.assertFalse(result.success)
        self.assertIn("positive integers", result.message)
        self.assertIsNone(self.service.get_open_round())

    def test_create_round_rejects_negative_candidate_id_cleanly(self) -> None:
        result = self.service.create_round(candidate_suggestion_ids=[-1, 2])

        self.assertFalse(result.success)
        self.assertIn("positive integers", result.message)
        self.assertIsNone(self.service.get_open_round())

    def test_create_round_rejects_boolean_candidate_id(self) -> None:
        result = self.service.create_round(candidate_suggestion_ids=[True, 2])
...
class VoteServiceDatabaseHistoryTests(unittest.TestCase):
    ...
    def test_recent_closed_rounds_rejects_zero_limit(self) -> None:
        with self.assertRaisesRegex(ValueError, "positive integer"):
            self.service.get_recent_closed_rounds(0)

    def test_recent_closed_rounds_rejects_negative_limit(self) -> None:
        with self.assertRaisesRegex(ValueError, "positive integer"):
            self.service.get_recent_closed_rounds(-1)

    def test_recent_closed_rounds_rejects_boolean_limit(self) -> None:
        with self.assertRaisesRegex(ValueError, "positive integer"):
            self.service.get_recent_closed_rounds(True)

    def test_recent_closed_rounds_rejects_invalid_database_id(self) -> None:
        with self.assertRaisesRegex(ValueError, "database_id"):
            self.service.get_recent_closed_rounds(5, database_id=0)
