...
class JsonVoteRepositoryTests(unittest.TestCase):
    ...
    def test_save_then_load_round_trips_reminder_minutes_before_close(self) -> None:
        self.repository.save([VoteRound(id=1, reminder_minutes_before_close=240)], next_round_id=2)

        result = self.repository.load()
        self.assertEqual(result.rounds[0].reminder_minutes_before_close, 240)

    def test_loading_a_legacy_hours_field_converts_to_the_equivalent_minutes(self) -> None:
        now = utc_now()
        legacy_json = f"""
        {{
          "next_round_id": 2,
    ...
    def test_loading_a_file_without_reminder_fields_defaults_them_to_none(self) -> None:
        ...
        self.assertIsNone(loaded.reminder_minutes_before_close)
...
class VoteRepositoryLegacyRotationFieldTests(unittest.TestCase):
    """Rotation Removal: VoteRound.rotation_id no longer exists -- a
    record written before this milestone may still have a "rotation_id"
    key in its JSON, which must be silently ignored on load rather than
    causing a crash.
    """
    ...
    def test_legacy_round_with_a_rotation_id_still_loads(self) -> None:
        now = utc_now()
        self.file_path.write_text(
            '{"next_round_id": 2, "rounds": [{"id": 1, "status": "open", '
            '"visibility": "visible", "created_at": "' + now.isoformat() + '", '
            '"closes_at": null, "winning_suggestion_id": null, "database_id": 9, '
            '"rotation_id": 3, "votes": []}]}',
            encoding="utf-8",
        )
        loaded_round = self.repository.load().rounds[0]
        self.assertFalse(hasattr(loaded_round, "rotation_id"))
