"""Tests for FR-030's /join watch party wiring in bot.py.
...
fallback to wherever /join watch party was invoked) -- see
...
class HandleJoinWatchPartyPermissionTests(MembershipCommandTestCase):
    """FR-030: Everyone can execute /join watch party -- no role required."""
...
class HandleJoinWatchPartyModeTests(MembershipCommandTestCase):
    ...
    async def test_approval_mode_never_uses_the_log_channel_or_invocation_channel(self) -> None:
        # Even with a log channel configured and /join watch party
